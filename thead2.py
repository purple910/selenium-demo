        ...
        while True:
            ...
            while True:
                try:
                    ...
                    filename = './' + xinxi[0] + '/' + xinxi[1] + '/' + xinxi[1] + x + '.csv'
                    # 以写的形式打开文件 newline为空  防止出现多余的空行
                    with open(filename, 'a', encoding='utf-8', newline='') as csvfile:
                        # 创建一个操作此文件的对象
                        writer = csv.writer(csvfile)
                        # 对得到的数据压缩（因为数据都是一样的条数且一一对应的）以后再对它迭代
                        for gangwei, gongsi, sange, xueli, jingyan, xinzi, chengshi, riqi in zip(gangweis, gongsis,
                                                                                                 sanges,
                                                                                                 xuelis, jingyans,
                                                                                                 xinzis,
                                                                                                 chengshis, riqis, ):
...
if __name__ == '__main__':
    ...
    # 创建线程
    for i in ['127.0.0.1:9223', '127.0.0.1:9224', '127.0.0.1:9222']:
        x = i
        x = i
        dt = Get_xinxi(x)
        threads.append(dt)
